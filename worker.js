// ═══════════════════════════════════════════════════════════════════
// EDH Power Level — Cloudflare Worker Proxy
// spellbook-proxy.work-macarthur.workers.dev
// Routes: /moxfield, /archidekt, /find-my-combos, /edhrec, /claude
// ═══════════════════════════════════════════════════════════════════
...
export default {
  async fetch(request, env) {
    ...
    if (path === '/moxfield') {
      ...
      if (!deckId) return errorResponse('Missing id param', 400);
      ...
      try {
        ...
        if (!resp.ok) {
          return errorResponse(
            `Moxfield returned ${resp.status} — make sure the deck is set to Public`,
            resp.status
          );
        }
        ...
        if (lines.length === 0) {
          return errorResponse('No cards found — is this deck public?');
        }

        return jsonResponse({ name: deckName, list: lines.join('\n'), count: lines.length });

      } catch (e) {
        return errorResponse('Failed to fetch from Moxfield: ' + e.message);
      }
    }
    ...
    if (path === '/archidekt') {
      ...
      try {
        ...
        if (!resp.ok) {
          return errorResponse(
            `Archidekt returned ${resp.status} — make sure the deck is set to Public`,
            resp.status
          );
        }
        ...
        return errorResponse('Failed to fetch from Archidekt: ' + e.message);
      }
    }
    ...
    if (path === '/find-my-combos') {
      ...
      try {
        ...
        return errorResponse('Spellbook error: ' + e.message);
      }
    }
    ...
    if (path === '/edhrec') {
      ...
      if (!commander) return errorResponse('Missing commander param', 400);
      ...
      try {
        ...
        const resp = await fetch(`https://json.edhrec.com/pages/commanders/${slug}.json`, {
          headers: { 'User-Agent': 'EDHPowerLevel/1.0 (edhpowerlevel.ca)' },
        });

        if (!resp.ok) return errorResponse(`EDHREC returned ${resp.status}`, resp.status);
        const data = await resp.json();
        return jsonResponse(data);
      } catch (e) {
        return errorResponse('EDHREC error: ' + e.message);
      }
    }
    ...
    if (path === '/claude') {
      ...
      try {
        ...
        return errorResponse('Claude API error: ' + e.message);
      }
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        available: ['/moxfield', '/archidekt', '/find-my-combos', '/edhrec', '/claude']
      }),
